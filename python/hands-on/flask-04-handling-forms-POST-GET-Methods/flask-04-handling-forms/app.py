...
@app.route("/")
def home():
    return render_template("main.html", name="Mert")
...
def login():
    if request.method=="POST":
        my_name = request.form['username']
        my_pass = request.form['password']
        if my_pass == 'awesome-09-cohort':
            return render_template('secure.html', user = my_name.title())
        else:
            return render_template('login.html', control = True, user = my_name.title())
    else:
